...
check_for_image_updates: ## Check for image updates
	...
	@./tools/check_for_image_updates.sh "$$(awk -F= '/MYSQL_IMAGE=container/&&!/aarch64/{print $$2}' generate_env.sh)" container-registry.oracle.com/mysql/community-server:9.7
	@./tools/check_for_image_updates.sh "$$(awk -F= '/MYSQL_IMAGE=container/&&/aarch64/{print $$2}' generate_env.sh)" container-registry.oracle.com/mysql/community-server:9.7-aarch64
